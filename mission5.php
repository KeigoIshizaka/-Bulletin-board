...
<?php 
	...
	$stmt->execute();
    }else if(!empty($_POST["name"]) && !empty($_POST["message"]) && !empty($_POST["pass"])){
    	...
    	$stmt->bindParam(':pass', $pass, PDO::PARAM_STR);
            	...
            	$pass =$_POST["pass"];
    	$stmt->execute();
    }else if(!empty($_POST["change"]) && !empty($_POST["pass"])){
        //編集番号とパスワードがあっている時に投稿フォームに表示させる
        $id = $_POST["change"];
        $pass=$_POST["pass"];
